...
fn gen_expr_stmt<'a>(stmt: &ExprStmt<'a>, context: &mut Context<'a>) -> PrintItems {
  // zts fork: `g()?;` needs its semicolon — see ends_with_zts_try.
  if context.config.semi_colons.is_true() || stmt.parent().is::<DoWhileStmt>() || zts_requires_semi_colon(stmt.into()) {
  ...
  fn gen_inner<'a>(stmt: &ExprStmt<'a>, context: &mut Context<'a>) -> PrintItems {
    ...
    if context.config.semi_colons.is_true() || zts_requires_semi_colon(stmt.into()) {
      items.push_sc(sc!(";"));
    }
    items
  }
...
fn gen_return_stmt<'a>(node: &ReturnStmt<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  // zts fork: `return e?;` needs its semicolon — see ends_with_zts_try.
  if context.config.semi_colons.is_true() || zts_requires_semi_colon(node.into()) {
    items.push_sc(sc!(";"));
  }
  items
}
...
/// zts fork: must this statement keep its semicolon even under `semiColons:
/// "asi"`? Two zts constructs make the terminator load-bearing.
///
/// 1. A statement ending in the `?` try operator. The operator is recognised
///    only where a ternary is impossible — the token straight after it must be
///    `;`, `)`, `,` or `]`. At statement level that token is the semicolon, so
///    dropping it does not change style, it changes the parse: `const v = g()?`
///    followed by a newline reads `?` as the start of a conditional expression
///    and the file stops compiling. The three shapes checked are exactly the
///    three the language permits the operator in — a `const`/`let` initializer,
///    a `return` argument, and a bare expression statement.
///
/// 2. The last statement of an expression block, because what follows it is the
///    block's tail EXPRESSION. dprint's usual defence against ASI hazards is to
///    prefix the *following statement* with a semicolon, and a tail is not a
///    statement, so nothing would defend this boundary. A tail beginning with a
///    template literal, `(`, `[`, `+`, `-` or `/` simply continues the previous
///    line: the block in
///
///        } else {
///          const neg = -n
...
///        }
///
///    parses as `-n` tagged with a template, leaving the block with no tail at
///    all. Keeping the terminator is both the smaller diff and the more
///    readable output compared with a leading `;` on the tail.
fn zts_requires_semi_colon(node: Node) -> bool {
  ends_with_zts_try(node) || is_last_stmt_of_zts_expr_block(node)
}

fn ends_with_zts_try(node: Node) -> bool {
  fn expr_ends_with_try(expr: &Expr) -> bool {
    matches!(expr, Expr::ZtsTry(_))
  }

  match node {
    Node::ExprStmt(stmt) => expr_ends_with_try(&stmt.expr),
    Node::ReturnStmt(stmt) => stmt.arg.as_ref().is_some_and(expr_ends_with_try),
    Node::VarDecl(decl) => decl.decls.last().and_then(|d| d.init.as_ref()).is_some_and(expr_ends_with_try),
    _ => false,
  }
}

fn is_last_stmt_of_zts_expr_block(node: Node) -> bool {
  // `Stmt` and `Decl` are enums rather than view nodes, so a statement's parent
  // is the block itself with no wrapper in between
  let Some(Node::ZtsExprBlock(block)) = node.parent() else {
    return false;
  };
  block.stmts.last().is_some_and(|last| last.range() == node.range())
}

fn requires_semi_colon_for_var_or_using_decl(node: Node, context: &mut Context) -> bool {
  // zts fork: a `?` try operator at the end makes the semicolon load-bearing.
  let use_semi_colons = context.config.semi_colons.is_true() || zts_requires_semi_colon(node);
  use_semi_colons
    && match node.parent() {
...
fn gen_match_variant_pat<'a>(node: &MatchVariantPat<'a>, context: &mut Context<'a>) -> PrintItems {
  ...
  // renames (`{ radius: r }`), defaults and rest elements for free.
  //
  // The braces are mandatory in the grammar, so a pattern that binds nothing is
  // written `Circle {}` and arrives here as an empty object pattern. The AST
  // models the binding as optional anyway; if one ever arrives without it, an
  // empty pair of braces is printed rather than a bare name, because a bare
  // name would not parse back.
  let mut items = PrintItems::new();
  items.extend(gen_node(node.name.into(), context));
  items.push_space();
  match node.binding {
    Some(binding) => items.extend(gen_node(binding.into(), context)),
